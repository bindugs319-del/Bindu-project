    ...
    while True:
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                cutoff_start = datetime.now(timezone.utc).replace(tzinfo=None)
                ...
                stmt2 = select(PurchaseOrder, User).join(User, PurchaseOrder.user_id == User.id).where(
                    (PurchaseOrder.payment_completed_at.is_(None)) &
                    (PurchaseOrder.due_date < datetime.now(timezone.utc).replace(tzinfo=None))
                )
                ...
                for sr, po in tasks:
                    if po.vendor_email:
                        try:
                            ...
                            sr.sent_at = datetime.now(timezone.utc).replace(tzinfo=None)
                            await session.commit()
                        except Exception as e:

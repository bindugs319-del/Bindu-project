...
export default function Defaulters() {
  ...
  // ?context=po or ?context=invoice — set by the Quick Actions button on
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      business_gstin: form.business_gstin ? form.business_gstin.trim().toUpperCase() : null,
  ...
  const handleView = (row) => {
    setSelectedCase(row)
    setDocUpdate({
      documents_drive_folder: row.documents_drive_folder || '',
  ...
  const handleDocSave = async () => {
    ...
    if (res.ok) {
      setRows((prev) => prev.map((r) => r.id === selectedCase.id ? { ...r, ...docUpdate } : r))
      setSelectedCase((prev) => prev ? { ...prev, ...docUpdate } : prev)
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <input
                  ...
                  onChange={handleChange}
                  placeholder={context === 'po' ? 'PO #' : 'Invoice #'}
                  required
                />
                <input
                  className="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-primary-500 focus:ring-2 focus:ring-primary-200"
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {!loading && visibleRows.length === 0 && (
                  <tr>
                    <td colSpan={7} className="py-3 text-center text-gray-500 py-4">{context ? `No ${context === 'po' ? 'PO' : 'Invoice'}-related cases yet.` : 'No cases yet.'}</td>
                  </tr>
                )}
                {visibleRows.map((row) => (
                  ...
                  <div className="mt-3 space-y-3">
                    ...
                    <div className="grid md:grid-cols-2 gap-3">
                      <input
                        className="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-primary-500 focus:ring-2 focus:ring-primary-200"
                        name="documents_drive_folder"
                        value={docUpdate.documents_drive_folder}
